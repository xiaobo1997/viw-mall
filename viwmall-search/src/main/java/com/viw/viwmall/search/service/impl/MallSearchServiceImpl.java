...
import com.viw.viwmall.search.feign.ProductFeignService;
import com.viw.viwmall.search.service.MallSearchService;
import com.viw.viwmall.search.vo.AttrResponseVo;
import com.viw.viwmall.search.vo.BrandVo;
 ...
 * @Date: 2021/3/6 20:40
 * @description: 获取各个服务的检索商品信息的请求业务
 */
@Service
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            //2、执行检索请求
            SearchResponse response = client.search(searchRequest, ViwmallElasticSearchConfig.COMMON_OPTIONS);
    ...
    private SearchRequest buildSearchRequrest(SearchParam param) {
        ...
        //1.2、bool - filter - 按照所有指定的属性进行查询  多个过滤
...
//                <500
        ...
        System.out.println("DSL:无聚合分析的"+sourceBuilder.toString());
    ...
    private SearchResult buildSearchResult(SearchResponse response, SearchParam param) {
        ...
        if (hits.getHits() != null && hits.getHits().length > 0) {
            for (SearchHit hit : hits.getHits()) {
                String sourceAsString = hit.getSourceAsString();//获取命中的hit中的第一条真正的内容
                SkuEsModel esModel = JSON.parseObject(sourceAsString, SkuEsModel.class);
                if(!StringUtils.isEmpty(param.getKeyword())){
                    //取出检索结果中高亮的属性 并重新赋值esModels
        ...
        //2、当前所有商品涉及到的所有属性信息  这是一个嵌入式的聚合，聚合又聚合
        List<SearchResult.AttrVo> attrVos = new ArrayList<>();
        ParsedNested attr_agg = response.getAggregations().get("attr_agg");
        ParsedLongTerms attr_id_agg = attr_agg.getAggregations().get("attr_id_agg");//聚合中的 attr_agg中的attr_id_agg聚会查询结果
...
//        //4、当前所有商品涉及到的所有分类信息
        ...
        for (Terms.Bucket bucket : buckets) {
            ...
            // 得到 catalog_name_agg聚合的第一个buckets的第一个元素的数据 就是 聚合查询中的分类name
